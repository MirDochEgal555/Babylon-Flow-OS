...
const archiveDB = {
  open() { return new Promise((resolve, reject) => { const request = indexedDB.open(ARCHIVE_DB, ARCHIVE_DB_VERSION); request.onupgradeneeded = () => { const db = request.result; if (!db.objectStoreNames.contains('archive')) db.createObjectStore('archive'); if (!db.objectStoreNames.contains('photos')) db.createObjectStore('photos', { keyPath: 'id' }); }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); },
...
const photoStore = {
  async all() { const db = await archiveDB.open(); return new Promise((resolve, reject) => { const request = db.transaction('photos').objectStore('photos').getAll(); request.onsuccess = () => resolve(request.result || []); request.onerror = () => reject(request.error); }); },
  async add(photo) { const db = await archiveDB.open(); return new Promise((resolve, reject) => { const request = db.transaction('photos', 'readwrite').objectStore('photos').put(photo); request.onsuccess = () => resolve(); request.onerror = () => reject(request.error); }); },
...
async function loadArchive() {
  ...
  try { photos = await photoStore.all(); } catch { photos = []; }
...
function renderMarket() {
  ...
  $('#marketCard').innerHTML = `<div class="market-main"><span class="micro-label">FINAL MARKET CLOSE · 23 SEP 2026</span><p><strong class="up">${escapeHTML(best.alias)} · FINAL LEADER</strong><br>Markets are closed. These standings remain subject only to an evidence-backed HQ correction.</p></div><div class="market-stats"><div><span class="micro-label">TOP ASSET</span><b>${escapeHTML(best.alias)}</b></div><div><span class="micro-label">CHAOS INDEX</span><b>${Number(chaos.stats.chaos)}/100</b></div><div><span class="micro-label">SHOE RESERVE</span><b>02</b></div></div>`;
}
...
function clearPhotoUrls() { photoUrls.forEach(url => URL.revokeObjectURL(url)); photoUrls = []; }
function renderPhotoAlbum() { clearPhotoUrls(); const grid = $('#photoGrid'); if (!grid) return; if (!photos.length) { grid.innerHTML = '<p class="lede">No photos archived on this device yet. Add the evidence.</p>'; return; } grid.innerHTML = photos.map(photo => { const url = URL.createObjectURL(photo.blob); photoUrls.push(url); return `<figure class="photo-card"><img src="${url}" alt="${escapeHTML(photo.caption || photo.fileName || 'Babylon Flow photo')}" loading="lazy"><figcaption><span>${escapeHTML(photo.caption || 'UNTITLED EVIDENCE')}</span><small>${escapeHTML(photo.addedAt)}</small><button class="text-button" data-delete-photo="${escapeHTML(photo.id)}">DELETE</button></figcaption></figure>`; }).join(''); document.querySelectorAll('[data-delete-photo]').forEach(button => button.onclick = async () => { const id = button.dataset.deletePhoto; if (!confirm('Delete this photo from this device’s Babylon archive?')) return; try { await photoStore.remove(id); photos = photos.filter(photo => photo.id !== id); renderPhotoAlbum(); toast('PHOTO REMOVED FROM THIS DEVICE'); } catch { toast('PHOTO COULD NOT BE REMOVED'); } }); }
async function photoAlbumModal() { openModal(`<h2 id="modalTitle">BABYLON PHOTO ALBUM</h2><p class="lede">Photos are stored in this browser’s archive. They stay on this device unless you use a shared photo service separately.</p><form id="photoForm"><div class="form-grid"><div class="full"><label>SELECT PHOTO</label><input required name="photo" type="file" accept="image/*"></div><div class="full"><label>CAPTION (OPTIONAL)</label><input name="caption" maxlength="100" placeholder="e.g. Ibiza survival evidence"></div></div><button class="submit-button">ADD PHOTO TO ALBUM →</button></form><div class="photo-album-heading"><span class="micro-label">${photos.length} / ${PHOTO_LIMIT} PHOTOS ON THIS DEVICE</span></div><div class="photo-grid" id="photoGrid"><p class="lede">Loading album…</p></div>`, 'album', true); renderPhotoAlbum(); $('#photoForm').onsubmit = async event => { event.preventDefault(); const form = new FormData(event.target), file = form.get('photo'); if (!(file instanceof File) || !file.type.startsWith('image/')) { toast('SELECT A VALID IMAGE FILE'); return; } if (file.size > PHOTO_MAX_BYTES) { toast('PHOTO TOO LARGE · MAX 12 MB'); return; } if (photos.length >= PHOTO_LIMIT) { toast('ALBUM LIMIT REACHED · 200 PHOTOS'); return; } const submit = event.target.querySelector('button'); submit.disabled = true; submit.textContent = 'ARCHIVING PHOTO…'; const record = { id:`photo-${Date.now()}-${Math.random().toString(16).slice(2)}`, blob:file, caption:cleanText(form.get('caption'), '', 100), fileName:cleanText(file.name, 'Babylon photo', 120), addedAt:eventTime() }; try { await photoStore.add(record); photos.unshift(record); renderPhotoAlbum(); event.target.reset(); toast('PHOTO ARCHIVED ON THIS DEVICE'); } catch { toast('PHOTO COULD NOT BE ARCHIVED'); } finally { submit.disabled = false; submit.textContent = 'ADD PHOTO TO ALBUM →'; } }; }
function renderPhotoAlbum() { clearPhotoUrls(); const grid = $('#photoGrid'); if (!grid) return; const published = (BABYLON_PHOTOS || []).map(photo => ({ ...photo, published: true })), album = [...published, ...photos]; if (!album.length) { grid.innerHTML = '<p class="lede">No photos archived yet. Add JPEGs to the shared photos folder or upload local evidence.</p>'; return; } grid.innerHTML = album.map(photo => { const url = photo.published ? photo.src : URL.createObjectURL(photo.blob); if (!photo.published) photoUrls.push(url); const caption = photo.caption || photo.fileName || 'Babylon Flow photo'; return `<figure class="photo-card"><img src="${escapeHTML(url)}" alt="${escapeHTML(caption)}" loading="lazy"><figcaption><span>${escapeHTML(caption)}</span><small>${escapeHTML(photo.addedAt || 'PUBLISHED ALBUM')}</small>${photo.published ? '<small>SHARED PHOTO</small>' : `<button class="text-button" data-delete-photo="${escapeHTML(photo.id)}">DELETE</button>`}</figcaption></figure>`; }).join(''); document.querySelectorAll('[data-delete-photo]').forEach(button => button.onclick = async () => { const id = button.dataset.deletePhoto; if (!confirm('Delete this photo from this device’s Babylon archive?')) return; try { await photoStore.remove(id); photos = photos.filter(photo => photo.id !== id); renderPhotoAlbum(); toast('PHOTO REMOVED FROM THIS DEVICE'); } catch { toast('PHOTO COULD NOT BE REMOVED'); } }); }
function photoAlbumModal() { const publishedCount = (BABYLON_PHOTOS || []).length; openModal(`<h2 id="modalTitle">BABYLON PHOTO ALBUM</h2><p class="lede">JPEGs and PNGs in the shared <code>photos/</code> folder appear for everyone after deployment. Browser uploads stay on this device.</p><form id="photoForm"><div class="form-grid"><div class="full"><label>SELECT JPEG OR PNG</label><input required name="photo" type="file" accept="image/jpeg,image/png"></div><div class="full"><label>CAPTION (OPTIONAL)</label><input name="caption" maxlength="100" placeholder="e.g. Ibiza survival evidence"></div></div><button class="submit-button">ADD LOCAL PHOTO →</button></form><div class="photo-album-heading"><span class="micro-label">${publishedCount} SHARED · ${photos.length} / ${PHOTO_LIMIT} LOCAL PHOTOS</span></div><div class="photo-grid" id="photoGrid"><p class="lede">Loading album…</p></div>`, 'album', true); renderPhotoAlbum(); $('#photoForm').onsubmit = async event => { event.preventDefault(); const form = new FormData(event.target), file = form.get('photo'); if (!(file instanceof File) || !['image/jpeg', 'image/png'].includes(file.type)) { toast('SELECT A JPEG OR PNG IMAGE'); return; } if (file.size > PHOTO_MAX_BYTES) { toast('PHOTO TOO LARGE · MAX 12 MB'); return; } if (photos.length >= PHOTO_LIMIT) { toast('ALBUM LIMIT REACHED · 200 PHOTOS'); return; } const submit = event.target.querySelector('button'); submit.disabled = true; submit.textContent = 'ARCHIVING PHOTO…'; const record = { id:`photo-${Date.now()}-${Math.random().toString(16).slice(2)}`, blob:file, caption:cleanText(form.get('caption'), '', 100), fileName:cleanText(file.name, 'Babylon photo', 120), addedAt:eventTime() }; try { await photoStore.add(record); photos.unshift(record); renderPhotoAlbum(); event.target.reset(); toast('PHOTO ARCHIVED ON THIS DEVICE'); } catch { toast('PHOTO COULD NOT BE ARCHIVED'); } finally { submit.disabled = false; submit.textContent = 'ADD LOCAL PHOTO →'; } }; }
...
function wrappedModal() { const ranked=[...activePeople()].sort((a,b)=>b.stock-a.stock), liability=[...people].sort((a,b)=>b.stats.chaos-a.stats.chaos)[0], km=events.length*3.7+31, phrase=quotes[0]?.quote||'No quote on record', late=events.filter(e=>/0[0-5]:/.test(e.time)).length, comeback=ranked[0]; openModal(`<div class="wrapped-hero"><span class="micro-label">BABYLON FLOW PRESENTS · FINAL EDITION</span><h2 id="modalTitle">TRIP WRAPPED<br>2026</h2><p class="lede">España 2026 concluded on 23 September. The record is sealed pending evidence-backed corrections.</p></div><div class="wrapped-stats"><div><span class="micro-label">BIGGEST LIABILITY</span><b>${escapeHTML(liability.alias)}</b></div><div><span class="micro-label">KM TRAVELLED*</span><b>${km.toFixed(0)}</b></div><div><span class="micro-label">LATEST BEDTIME</span><b>${late?'05:42':'ON FILE'}</b></div><div><span class="micro-label">LOGGED INCIDENTS</span><b>${events.length}</b></div><div><span class="micro-label">FINAL MARKET LEADER</span><b>${escapeHTML(comeback.alias)}</b></div><div><span class="micro-label">QUOTE OF THE TRIP</span><b style="font-size:16px">“${escapeHTML(phrase)}”</b></div></div><h3 style="margin:24px 0 10px;font:18px var(--display)">FINAL STANDINGS</h3><div class="stock-table">${ranked.map((person,index)=>`<div class="stock-row"><b>#${index+1} · ${escapeHTML(person.alias)}</b><span>€${person.stock.toFixed(1)}</span><strong class="up">${person.stats.xp} XP</strong><span>STANDING LOCKED</span></div>`).join('')}</div><p class="lede" style="margin-top:16px">*Estimated from incident density, taxi lore and an unverified beach walk.</p>`, 'wrapped', true); }

function calendarMonth(year, month) {
  const eventByDate = new Map(BABYLON_CALENDAR_EVENTS.map(event => [event.date, event]));
  ...
  const cells = Array.from({ length: offset + daysInMonth }, (_, index) => {
    if (index < offset) return '<div class="calendar-day empty" aria-hidden="true"></div>';
    const day = index - offset + 1, date = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    const event = eventByDate.get(date);
    return `<article class="calendar-day${event ? ' has-event' : ''}"><span>${day}</span>${event ? `<b>${escapeHTML(event.title)}</b><small>${escapeHTML(event.attendees)}</small>` : ''}</article>`;
  }).join('');
  return `<section class="calendar-month"><h3>${firstDay.toLocaleString('en-GB', { month: 'long', year: 'numeric' })}</h3><div class="calendar-weekdays"><span>MON</span><span>TUE</span><span>WED</span><span>THU</span><span>FRI</span><span>SAT</span><span>SUN</span></div><div class="calendar-grid">${cells}</div></section>`;
}

function calendarModal() {
  const recorded = BABYLON_CALENDAR_EVENTS.length;
  openModal(`<h2 id="modalTitle">TRIP CALENDAR</h2><p class="lede">${recorded} logged days from the shared trip diary. Suggestions are emailed to Babylon HQ for review before they enter the permanent record.</p><div class="calendar-wrap">${calendarMonth(2026, 7)}${calendarMonth(2026, 8)}</div><form id="calendarSuggestionForm" class="calendar-suggestion"><h3>SUGGEST AN EVENT</h3><p class="lede">This sends a proposal by email; it does not automatically change the final ledger.</p><div class="form-grid"><div><label>DATE</label><input required name="date" type="date" min="2026-08-22" max="2026-09-23"></div><div><label>LOCATION</label><input required name="location" maxlength="60" placeholder="e.g. Denia"></div><div class="full"><label>EVENT TITLE</label><input required name="title" maxlength="80" placeholder="e.g. The Great Taxi Debate"></div><div class="full"><label>WHAT HAPPENED?</label><textarea required name="details" maxlength="500" placeholder="Give Babylon HQ enough evidence to add it correctly."></textarea></div><div class="full"><label>ATTENDEES (OPTIONAL)</label><input name="attendees" maxlength="180" placeholder="Names, separated by commas"></div></div><button class="submit-button">EMAIL EVENT SUGGESTION →</button><p class="calendar-email-status" id="calendarEmailStatus" aria-live="polite"></p></form>`, 'calendar', true);
  $('#calendarSuggestionForm').onsubmit = async event => {
    event.preventDefault();
    const form = event.currentTarget, data = new FormData(form), submit = form.querySelector('button'), status = $('#calendarEmailStatus');
    submit.disabled = true; submit.textContent = 'SENDING TO HQ…'; status.textContent = '';
    try {
      const result = await emailLog('Calendar event suggestion', { Date: data.get('date'), Title: data.get('title'), Location: data.get('location'), 'What happened': data.get('details'), Attendees: data.get('attendees') || 'Not supplied' });
      form.reset();
      status.textContent = result.needsActivation ? 'Suggestion logged. Check the receiving inbox to activate email delivery.' : 'Suggestion emailed to Babylon HQ for review.';
      toast('EVENT SUGGESTION SENT');
    } catch {
      status.textContent = 'Email could not be sent. Please try again later.';
      toast('EVENT SUGGESTION NOT SENT');
    } finally { submit.disabled = false; submit.textContent = 'EMAIL EVENT SUGGESTION →'; }
  };
}

// The trip is over: late memories are welcome, but they cannot silently alter
// final XP, stats, or stock. Corrections remain an HQ-reviewed workflow.
function eventModal() { openModal(`<h2 id="modalTitle">ADD LATE MEMORY</h2><p class="lede">Record something the group remembered after the trip. Final standings will not change here.</p><form id="eventForm"><div class="form-grid"><div><label>PEOPLE INVOLVED</label><select name="people" multiple required size="4">${activePeopleOptions()}</select></div><div><label>CATEGORY</label><select name="category"><option>RECOVERED LORE</option><option>PARTY</option><option>CLUB</option><option>BEACH</option><option>FOOD</option><option>SPORT</option><option>LEGENDARY</option></select></div><div class="full"><label>INCIDENT TITLE</label><input required name="title" autocomplete="off" placeholder="The thing we forgot until now" maxlength="55"></div><div class="full"><label>WHAT HAPPENED</label><textarea name="description" required placeholder="Keep it brief. The archive is listening." maxlength="180"></textarea></div><div><label>LOCATION</label><input name="location" placeholder="JÁVEA HQ" maxlength="30"></div><div><label>WHEN DID IT HAPPEN?</label><input name="occurredAt" placeholder="e.g. 22 SEP · 03:40" maxlength="30"></div></div><button class="submit-button">ADD TO THE ARCHIVE →</button></form>`, 'event'); $('#eventForm').onsubmit = event => { event.preventDefault(); const data = new FormData(event.target), ids=data.getAll('people'), involved=people.filter(person=>ids.includes(person.id)); if (!involved.length) return; const time=(data.get('occurredAt') || eventTime()).toUpperCase(), location=(data.get('location') || 'FIELD UNKNOWN').toUpperCase(); rememberUndo(); events.unshift({ id:Date.now(), type:data.get('category'), title:data.get('title'), description:data.get('description'), location, people:ids, time, change:'LATE MEMORY · STANDINGS UNCHANGED' }); save('Late memory archived', data.get('title')); closeModal(); toast('MEMORY ARCHIVED · STANDINGS UNCHANGED'); notifyLog('Late memory archived', { 'People involved':involved.map(person=>person.alias).join(', '), Category:data.get('category'), Title:data.get('title'), 'What happened':data.get('description'), Location:location, 'When it happened':time, 'Final standings':'Unchanged' }); }; }

document.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>({event:eventModal,quote:quoteModal,quoteQuiz:quoteQuizModal,quest:questModal,court:courtModal,wheel:wheelModal,exchange:exchangeModal,npc:npcModal,var:varModal,lore:loreModal,duo:duoModal,morning:morningModal,calendar:calendarModal,wrapped:wrappedModal,album:photoAlbumModal}[b.dataset.open]()));
