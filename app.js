...
function currentArchive() { return { schemaVersion: ARCHIVE_SCHEMA_VERSION, updatedAt: new Date().toISOString(), source: BABYLON_ARCHIVE.source, people, events, quotes, npcs, morningReports, duoHistory, auditLog }; }
...
function questModal(quest = BABYLON_QUESTS[Math.floor(Math.random()*BABYLON_QUESTS.length)]) { openModal(`<h2 id="modalTitle">SIDE QUEST</h2><p class="lede">Optional mission. Mandatory lore potential.</p><div class="quest-card"><span class="rarity">${quest.rarity} ASSIGNMENT</span><h3>${quest.mission}</h3><div class="quest-details"><span>LOCATION <b>${quest.place}</b></span><span>REWARD <b>+${quest.xp} XP</b></span></div></div><div class="quest-actions"><button class="secondary-button" id="reroll">REROLL</button><button class="submit-button" id="completeQuest">COMPLETE QUEST</button></div>`, 'quest');
  $('#reroll').onclick=()=>questModal(); $('#completeQuest').onclick=()=>{ const p=people[Math.floor(Math.random()*people.length)];p.stats.xp+=quest.xp;events.unshift({id:Date.now(),type:'SIDE QUEST',title:'Side quest completed',description:`${p.alias} completed the mission: ${quest.mission}`,location:quest.place,time:'JUST NOW',change:`+${quest.xp} XP`});save('Side quest completed',quest.mission);closeModal();toast(`QUEST COMPLETE · ${p.alias} +${quest.xp} XP`);};
}
